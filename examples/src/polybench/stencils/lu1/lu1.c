...
for (t1=0;t1<=30;t1++) {
  ...
  for (t3=62*t1+95;t3<=63*t1+95;t3++) {
    a[(t1+1)*64 + (-63*t1+t3-32)] = a[(t1+1)*64 + (-63*t1+t3-32)] - a[(t1+1)*64 + t1]*a[t1*64 + (-63*t1+t3-32)];;
  }
  for (t2=62*t1+64;t2<=31*t1+1985;t2++) {
...
for (t1=31;t1<=61;t1++) {
  ...
  a[t1*64 + 63] = a[t1*64 + 63]/a[t1*64 + t1];;
  for (t3=64*t1+33;t3<=63*t1+95;t3++) {
    a[(t1+1)*64 + (-63*t1+t3-32)] = a[(t1+1)*64 + (-63*t1+t3-32)] - a[(t1+1)*64 + t1]*a[t1*64 + (-63*t1+t3-32)];;
  }
  for (t2=62*t1+64;t2<=31*t1+1985;t2++) {
    for (t3=ceild(32*t2-993,31);t3<=floord(-31*t1+32*t2+929,31);t3++) {
...
a[62*64 + 63] = a[62*64 + 63]/a[62*64 + 62];;
a[63*64 + 63] = a[63*64 + 63] - a[63*64 + 62]*a[62*64 + 63];;
/* End of CLooG code */
}
