...
void ex3(double *A, int n, int ch){   
  ...
  for(int i=1;i<n;i++){
	...
	if(ch==1) writeValue("ex3_var_addr_fi_1.csv",(void*)&addr);
	if(ch==2) writeValue("ex3_var_addr_fi_2.csv",(void*)&addr);
	if(ch==3) writeValue("ex3_var_addr_fi_3.csv",(void*)&addr);
	if(ch==4) writeValue("ex3_var_addr_fi_4.csv",(void*)&addr);
	if(ch==0) writeValue("ex3_var_addr.csv",(void*)&addr);
  }	
}
