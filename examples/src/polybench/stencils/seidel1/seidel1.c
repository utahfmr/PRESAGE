...
/* Start of CLooG code */
for (t1=0;t1<=63;t1++) {
  for (t2=2*t1+1;t2<=2*t1+62;t2++) {
    for (t3=1;t3<=62;t3++) {
      a[(-2*t1+t2)*64 + t3] = (a[((-2*t1+t2)-1)*64 + t3-1] + a[((-2*t1+t2)-1)*64 + t3] + a[((-2*t1+t2)-1)*64 + t3+1] + a[(-2*t1+t2)*64 +t3-1] + a[(-2*t1+t2)*64 + t3] + a[(-2*t1+t2)*64 + t3+1] + a[((-2*t1+t2)+1)*64 +t3-1] + a[((-2*t1+t2)+1)*64 + t3] + a[((-2*t1+t2)+1)*64 + t3+1])/9.0;;
    }
  }
}
/* End of CLooG code */
